...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            {{-- Add one transaction --}}
            <form action="{{ route('cash_transaction.add_temp', $location_id) }}" method="POST">
                @csrf
                <div class="row g-3">
                    {{-- Date --}}
                    <div class="col-lg-6">
                    ...
                    <div class="col-lg-6">
                        ...
                        <input type="number" step="0.01" name="amount"
                            class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount') }}" placeholder="{{ __('cash_transaction.amount') }}">
                        @error('amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <tbody>
                            @foreach ($transactions as $i => $tx)
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('cash_transaction.removeTemporary', [$location_id, $i]) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
